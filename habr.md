...
3 сентября в ods.ai скинули ссылки на соревнования на китайской платформе aichallenger. Артур Кузин [link] предложил всем желающим поучаствовать в short-video realtime classification. Меня заинтересовало данное соревнование, и так образовалась команда из меня и [Ильи Бойцова](https://www.linkedin.com/in/ilya-boytsov-2496a2128/). 
...
Необходимо было классифицировать короткие видео почти в реал-тайме, где каждое видео могло иметь более одного тега. Задача имела несколько особенностей:
...
Первый пункт был самый интересный, во-первых, не сразу стало ясно, на каком железе будет все считаться, во-вторых, ограничение по скорости так же включало в себя чтение видео. Таким образом можно было выделить две критичные части будущего решения: оптимизированное декодирование видео и сверточная сеть. Забегая вперед, скажу, что мы не уделили первой части должного внимания, за что и поплатились, но об этом ниже.
...
### Данные
Организаторы предоставили train и validation set, в первом было 125368 файлов, а во втором 30736 соответственно. В трейне только 11920 видео имели более 1 тега, а в валидации 3043. Видео были короткие, разной длины, еще и со случайным звуковым сопровождением.
...
Оказалось, что opencv для чтения видео - это очень медленно, а из-за того, что большинство видео имеет только 1 тег, BCEWithLogitsLoss практически не дает какого-то либо стабильного и хорошего результата. Так как в данных было много видео с одним тегом и предсказание лишнего тега сильно било по точности, мы решили для начала сконцетрироваться на предсказании только одного тега. 

opencv была заменена на PyAV и BCEWithLogitsLoss на обычный CrossEntropyLoss. Поменяли сеть на se_resnet50 для более быстрого инференса. Для обучения сети использовали learning_rate = 0.001, и уменьшали его на 10 каждые 30 эпох. В целом, после 60 эпох сеть уже показывала хорошую сходимость. Данное решение уже давало 0.78 точности на валидационном сете. Докера еще не было, мы расслабились и предвскушали легкое выход на ЛБ.
...
Мы не стали останавливаться на одной сетке, и накинули дополнительных классификаторов. В итоге, для обучения сети и классикаторов был выстроен следующий пайплайн: из видео извлекалось 3 случайных кадров, применялись стандартные аугментации и кадры подавались на вход сети, которая давала усредненное предсказание. Затем мы использовали обученную сеть для извлечения признаков из видео, которые брались после global pooling, и последующего обучения классикаторов. В качестве дополнительных классификаторов мы взяли линейную регрессию, svm, catboost. Из видео брались 6 случайных кадров, затем из них извлекались признаки, которые использовались для обучения классификаторов. Для финального предсказания использовался voting. 
