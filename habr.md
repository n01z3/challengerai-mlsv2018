...
Мы не стали останавливаться на одной сетке, и накинули дополнительных классификаторов. В итоге, для обучения сети и классикаторов был выстроен следующий пайплайн. Из видео извлекалось 3 случайных кадров, применялись стандартные аугментации и кадры подавались на вход сети. Сеть давала усредненное предсказание. Затем сеть использовалась для обучения дополнительных классификаторов. Из видео извлекались 5 случайных кадров, затем из них извлекались признаки, которые использовались для обучения классификаторов. Для финального предсказания использовался voting. 

Инференс выполнялся практически так же. Мы использовали 5 случайных кадров, обученную сеть и классификаторы. Получив ~0.85 на validation данных, мы отправили решение и ... даже не вошли в ЛБ, так как не влезли в ограничение по скорости. 



## Оптимизация

Именна та часть, на которую нужно было обратить пристальное внимание еще в самом начале. PyAV для быстрого декодирования видео решил проблему обучения сеток, но никак не инференса. Чтение 5 случайных кадров занимало 0.180 ms, и это только инфкенес. Чтение 3 случайных кадров, тоже выходило за граничные условия. В целом, декодирование видео на GPU - это ключ. Во-первых, можно использовать [nvvl](https://github.com/NVIDIA/nvvl) библиотеку от nvidia. Но из-за кучи зависимостей завести ее не удалось. Более того, она держит файлы открытыми. Когда есть всего пара сотен видео - это не проблема, но данных у нас было куда больше. Один из хаков - увеличить лимит на количество открытых файловых дескрипторов системы, но ломать чужой девбокс не хотелось. Во-вторых, можно собрать ffmpeg с возможностью декодирования видео на GPU, но ffmpeg не собрался из-за кучи зависимостей. На другом железе получилось собрать ffmpeg, но не получилось засабмитить.
