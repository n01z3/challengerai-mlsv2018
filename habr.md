...
Мы не стали останавливаться на одной сетке, и накинули дополнительных классификаторов. В итоге, для обучения сети и классикаторов был выстроен следующий пайплайн. Из видео извлекалось 3 случайных кадров, применялись стандартные аугментации и кадры подавались на вход сети. Сеть давала усредненное предсказание. Затем сеть использовалась для обучения дополнительных классификаторов. В качестве дополнительных классификаторов мы взяли линейную регрессию, svm, catboost. Из видео брались 6 случайных кадров, затем из них извлекались признаки, которые использовались для обучения классификаторов. Для финального предсказания использовался voting. 
...
В итоге, нам пришлось сабмитить более слабые модели. Решили сделать смешать validation и train, выделить из них новый train и validation, заново обучить все модели и использовать случайны кадр из видео при инерефенсе. Данное решение дало 0.81 точности на валидации, но только 0.79 на ЛБ. На этом соревнование для нас и закончилось.
